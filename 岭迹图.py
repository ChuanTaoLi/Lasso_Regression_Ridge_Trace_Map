...
'''加载数据集'''
data = pd.read_excel(r'D:\01代码收集\Lasso\Data.xlsx')
X = data.iloc[:, 0:-1]
y = data.iloc[:, -1]
...
# 需要标准化的列
columns_to_normalize = data.columns.difference(['protocol_type', 'service', 'flag', 'label'])

# 使用StandardScaler对需要标准化的列进行标准化
...
alpha_lasso = np.linspace(0, 1.5, 1000)
